...
namespace ApplicationManagers
{
    public class AssetBundleManager : MonoBehaviour
    {
        ...
        public static Object LoadAsset(string name, bool cached = false)
        {
            if (cached)
            {
                if (!_cache.ContainsKey(name))
                    _cache.Add(name, MainAssetBundle.LoadAsset(name));
                return _cache[name];
            }
            return MainAssetBundle.LoadAsset(name);
        }
        public static T InstantiateAsset<T>(string name) where T : Object
        {
            return (T)Instantiate(MainAssetBundle.LoadAsset(name));
        }


        public static T InstantiateAsset2<T>(string name) where T : Object
        {
            return (T)Instantiate(RRMAssetBundle.LoadAsset(name));
        }
        ...
        {
            return (T)Instantiate(MainAssetBundle.LoadAsset(name), position, rotation);
        }
