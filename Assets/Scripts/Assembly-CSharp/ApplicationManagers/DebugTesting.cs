	...
	internal class DebugTesting : MonoBehaviour
	{
		...
		public static void RunDebugCommand(string command)
		{
			...
			if (text == "spawnasset")
			{
				...
				Object.Instantiate(rotation: new Quaternion(float.Parse(array3[0]), float.Parse(array3[1]), float.Parse(array3[2]), float.Parse(array3[3])), original: FengGameManagerMKII.RCassets.LoadAsset(text2), position: position);
			}
			else
			{
