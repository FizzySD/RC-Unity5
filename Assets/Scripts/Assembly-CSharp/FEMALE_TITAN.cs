...
public class FEMALE_TITAN : Photon.MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		else if (grounded && !base.GetComponent<Animation>().IsPlaying("attack_jumpCombo_1"))
		{
			base.GetComponent<Rigidbody>().AddForce(new Vector3(0f - base.GetComponent<Rigidbody>().velocity.x, 0f, 0f - base.GetComponent<Rigidbody>().velocity.z), ForceMode.VelocityChange);
		}
	...
	public void lateUpdate()
	{
		...
		{
			if (base.GetComponent<Animation>()["run"].normalizedTime % 1f > 0.1f && base.GetComponent<Animation>()["run"].normalizedTime % 1f < 0.6f && stepSoundPhase == 2)
			{
				stepSoundPhase = 1;
			...
			if (base.GetComponent<Animation>()["run"].normalizedTime % 1f > 0.6f && stepSoundPhase == 1)
			{
				stepSoundPhase = 2;
	...
	private void startMain()
	{
		...
		else if (IN_GAME_MAIN_CAMERA.difficulty == 1)
		{
			...
			foreach (AnimationState item2 in base.GetComponent<Animation>())
			{
				item2.speed = 0.7f;
			}
		...
		else if (IN_GAME_MAIN_CAMERA.difficulty == 2)
		{
			...
			foreach (AnimationState item3 in base.GetComponent<Animation>())
			{
				item3.speed = 1f;
			}
	...
	private void turn(float d)
	{
		...
		base.GetComponent<Animation>()[turnAnimation].time = 0f;
	...
	public void update()
	{
		...
		{
			if (!attacked && attackCheckTime != 0f && base.GetComponent<Animation>()["attack_" + attackAnimation].normalizedTime >= attackCheckTime)
			{
				attacked = true;
			...
			if (attackCheckTimeA != 0f && ((base.GetComponent<Animation>()["attack_" + attackAnimation].normalizedTime >= attackCheckTimeA && base.GetComponent<Animation>()["attack_" + attackAnimation].normalizedTime <= attackCheckTimeB) || (!attackChkOnce && base.GetComponent<Animation>()["attack_" + attackAnimation].normalizedTime >= attackCheckTimeA)))
			{
				if (!attackChkOnce)
				{
					attackChkOnce = true;
			...
			if (attackAnimation == "jumpCombo_1" && base.GetComponent<Animation>()["attack_" + attackAnimation].normalizedTime >= 0.65f && !startJump && myHero != null)
			{
				startJump = true;
				...
				Vector3 vector2 = myHero.transform.position + myHero.GetComponent<Rigidbody>().velocity * num8 + Vector3.up * 0.5f * num3 * num8 * num8;
				float y3 = vector2.y;
				if (num7 < 0f || y3 - y2 < 0f)
				{
					idle();
			...
			{
				if (base.GetComponent<Animation>()["attack_" + attackAnimation].normalizedTime >= 1f && IsGrounded())
				{
					attack("jumpCombo_4");
				}
			}
		...
		{
			if (base.GetComponent<Animation>()["attack_grab_" + attackAnimation].normalizedTime >= attackCheckTimeA && base.GetComponent<Animation>()["attack_grab_" + attackAnimation].normalizedTime <= attackCheckTimeB && grabbedTarget == null)
			{
			...
			if (base.GetComponent<Animation>()["attack_grab_" + attackAnimation].normalizedTime > attackCheckTime && grabbedTarget != null)
			{
				justEatHero(grabbedTarget, currentGrabHand);
				grabbedTarget = null;
			}
			if (base.GetComponent<Animation>()["attack_grab_" + attackAnimation].normalizedTime >= 1f)
			{
				idle();
			}
		}
		else if (state == "turn")
		{
			...
			if (base.GetComponent<Animation>()[turnAnimation].normalizedTime >= 1f)
			{
				idle();
			}
		}
