...
public class FlareMovement : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (timer < 4f)
		{
			base.GetComponent<Rigidbody>().AddForce((base.transform.forward + base.transform.up * 5f) * Time.deltaTime * 5f, ForceMode.VelocityChange);
		}
		else
		{
			base.GetComponent<Rigidbody>().AddForce(-base.transform.up * Time.deltaTime * 7f, ForceMode.Acceleration);
		}
	}
}
