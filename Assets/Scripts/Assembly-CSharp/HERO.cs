...
internal class HERO : Photon.MonoBehaviour
{
	...
	{
		ThunderSpearL = (GameObject)UnityEngine.Object.Instantiate(FengGameManagerMKII.RCassets.LoadAsset("ThunderSpearProp"));
		ThunderSpearR = (GameObject)UnityEngine.Object.Instantiate(FengGameManagerMKII.RCassets.LoadAsset("ThunderSpearProp"));
	...
	private void applyForceToBody(GameObject GO, Vector3 v)
	{
		...
		GO.GetComponent<Rigidbody>().AddTorque(UnityEngine.Random.Range(-10f, 10f), UnityEngine.Random.Range(-10f, 10f), UnityEngine.Random.Range(-10f, 10f));
	}
	...
	private void bodyLean()
	{
		...
		if (!useGun && state == HERO_STATE.Attack && attackAnimation != "attack3_1" && attackAnimation != "attack3_2" && !IsFiringThunderSpear())
		{
			...
			float z2 = base.GetComponent<Rigidbody>().velocity.z;
			float x2 = Mathf.Sqrt(x * x + z2 * z2);
			float num = Mathf.Atan2(y, x2) * 57.29578f;
			targetRotation = Quaternion.Euler((0f - num) * (1f - Vector3.Angle(base.GetComponent<Rigidbody>().velocity, base.transform.forward) / 90f), facingDirection, 0f);
	...
	private void breakApart2(Vector3 v, bool isBite)
	{
		...
		gameObject.GetComponent<HERO_DEAD_BODY_SETUP>().init(currentAnimation, base.GetComponent<Animation>()[currentAnimation].normalizedTime, BODY_PARTS.ARM_R);
		if (!isBite)
		{
			...
			gameObject2.GetComponent<HERO_DEAD_BODY_SETUP>().init(currentAnimation, base.GetComponent<Animation>()[currentAnimation].normalizedTime, BODY_PARTS.UPPER);
			gameObject3.GetComponent<HERO_DEAD_BODY_SETUP>().init(currentAnimation, base.GetComponent<Animation>()[currentAnimation].normalizedTime, BODY_PARTS.LOWER);
			gameObject4.GetComponent<HERO_DEAD_BODY_SETUP>().init(currentAnimation, base.GetComponent<Animation>()[currentAnimation].normalizedTime, BODY_PARTS.ARM_L);
		...
		gameObject5.GetComponent<Renderer>().material = CharacterMaterials.materials[setup.myCostume._3dmg_texture];
		gameObject6.GetComponent<Renderer>().material = CharacterMaterials.materials[setup.myCostume._3dmg_texture];
		gameObject7.GetComponent<Renderer>().material = CharacterMaterials.materials[setup.myCostume._3dmg_texture];
		gameObject8.GetComponent<Renderer>().material = CharacterMaterials.materials[setup.myCostume._3dmg_texture];
		gameObject9.GetComponent<Renderer>().material = CharacterMaterials.materials[setup.myCostume._3dmg_texture];
	...
	private void bufferUpdate()
	{
		...
		{
			buffTime = 0f;
			if (currentBuff == BUFF.SpeedUp && base.GetComponent<Animation>().IsPlaying("run_sasha"))
			{
				crossFade("run", 0.1f);
			}
	...
	public void cache()
	{
		...
		baseRigidBody = base.GetComponent<Rigidbody>();
		maincamera = GameObject.Find("MainCamera");
		if (IN_GAME_MAIN_CAMERA.gametype != 0 && !base.photonView.isMine)
		{
			return;
		}
		baseAnimation = base.GetComponent<Animation>();
	...
	public string currentPlayingClipName()
	{
		foreach (AnimationState item in base.GetComponent<Animation>())
		{
			if (base.GetComponent<Animation>().IsPlaying(item.name))
			{
				return item.name;
			}
		}
	...
	private void dash(float horizontal, float vertical)
	{
		if (dashTime <= 0f && currentGas > 0f && !isMounted && _dashCooldownLeft <= 0f)
		{
			...
			base.GetComponent<Rigidbody>().rotation = rotation;
			...
			falseAttack();
			base.GetComponent<Rigidbody>().AddForce(dashV * 40f, ForceMode.VelocityChange);
	...
	private void erenTransform()
	{
		...
		eren_titan.GetComponent<Rigidbody>().velocity = base.GetComponent<Rigidbody>().velocity;
		base.GetComponent<Rigidbody>().velocity = Vector3.zero;
				...
				else
				{
					...
					else
					{
						...
						if (myHorse != null && (baseAnimation.IsPlaying("horse_geton") || baseAnimation.IsPlaying("air_fall")) && baseRigidBody.velocity.y < 0f && Vector3.Distance(myHorse.transform.position + Vector3.up * 1.65f, baseTransform.position) < 0.5f)
						{
							...
							isMounted = true;
							if (!base.GetComponent<Animation>().IsPlaying("horse_idle"))
							{
	...
	private float getLeanAngle(Vector3 p, bool left)
	{
		...
		num3 *= 0.1f;
		num3 *= 1f + Mathf.Pow(base.GetComponent<Rigidbody>().velocity.magnitude, 0.2f);
	...
	private void getOffHorse()
	{
		...
		base.GetComponent<Rigidbody>().AddForce(Vector3.up * 10f - base.transform.forward * 2f - base.transform.right * 1f, ForceMode.VelocityChange);
		unmounted();
	}
	...
	{
		if ((base.GetComponent<Animation>().IsPlaying(standAnimation) || base.GetComponent<Animation>().IsPlaying("run") || base.GetComponent<Animation>().IsPlaying("run_sasha")) && (currentBladeSta != totalBladeSta || currentBladeNum != totalBladeNum || currentGas != totalGas || leftBulletLeft != bulletMAX || rightBulletLeft != bulletMAX))
		{
	...
	public void hookToHuman(GameObject target, Vector3 hookPosition)
	{
		...
		if (grounded)
		{
			base.GetComponent<Rigidbody>().AddForce(Vector3.up * Mathf.Min(launchForce.magnitude * 0.2f, 10f), ForceMode.Impulse);
		}
		base.GetComponent<Rigidbody>().AddForce(launchForce * num * 0.1f, ForceMode.Impulse);
	}
	...
	public void launch(Vector3 des, bool left = true, bool leviMode = false)
	{
		...
		leviMode = ((base.GetComponent<Animation>().IsPlaying("attack5") || base.GetComponent<Animation>().IsPlaying("special_petra")) ? true : false);
		if (!leviMode)
		{
			falseAttack();
			idle();
		...
		if (!leviMode)
		{
			...
			base.GetComponent<Rigidbody>().AddForce(launchForce);
		}
	...
	private void launchLeftRope(RaycastHit hit, bool single, int mode = 0)
	{
		if (currentGas != 0f)
		{
			...
			if (mode == 1)
			{
				component.launch(vector * 3f, base.GetComponent<Rigidbody>().velocity, launcher_ref, true, base.gameObject, true);
			}
			else
			{
				component.launch(vector * 3f, base.GetComponent<Rigidbody>().velocity, launcher_ref, true, base.gameObject);
			}
	...
	private void launchRightRope(RaycastHit hit, bool single, int mode = 0)
	{
		if (currentGas != 0f)
		{
			...
			if (mode == 1)
			{
				component.launch(vector * 5f, base.GetComponent<Rigidbody>().velocity, launcher_ref, false, base.gameObject, true);
			}
			else
			{
				component.launch(vector * 3f, base.GetComponent<Rigidbody>().velocity, launcher_ref, false, base.gameObject);
			}
	...
	private void RPCHookedByHuman(int hooker, Vector3 hookPosition)
	{
		...
		if (Vector3.Distance(hookPosition, base.transform.position) < 15f)
		{
			...
			base.GetComponent<Rigidbody>().AddForce(-base.GetComponent<Rigidbody>().velocity * 0.9f, ForceMode.VelocityChange);
			float num = Mathf.Pow(launchForce.magnitude, 0.1f);
			if (grounded)
			{
				base.GetComponent<Rigidbody>().AddForce(Vector3.up * Mathf.Min(launchForce.magnitude * 0.2f, 10f), ForceMode.Impulse);
	...
	private void setHookedPplDirection()
	{
		...
		if (isRightHandHooked && isLeftHandHooked)
		{
			if (bulletLeft != null && bulletRight != null)
			{
				...
				if (normal.sqrMagnitude < 4f)
				{
					...
					if (useGun && state != HERO_STATE.Attack)
					{
						float current = (0f - Mathf.Atan2(base.GetComponent<Rigidbody>().velocity.z, base.GetComponent<Rigidbody>().velocity.x)) * 57.29578f;
		...
		else
		{
			...
			if (state != HERO_STATE.Attack)
			{
				float current3 = (0f - Mathf.Atan2(base.GetComponent<Rigidbody>().velocity.z, base.GetComponent<Rigidbody>().velocity.x)) * 57.29578f;
	...
	private void suicide2()
	{
		if (IN_GAME_MAIN_CAMERA.gametype != 0)
		{
			netDieLocal(base.GetComponent<Rigidbody>().velocity * 50f, false, -1, string.Empty);
	...
	private void throwBlades()
	{
		...
		gameObject.GetComponent<Renderer>().material = CharacterMaterials.materials[setup.myCostume._3dmg_texture];
		gameObject2.GetComponent<Renderer>().material = CharacterMaterials.materials[setup.myCostume._3dmg_texture];
	...
	public void update2()
	{
		...
		else if (!titanForm && !isCannon)
		{
			...
			if (state == HERO_STATE.Idle && !GameMenu.InMenu())
			{
				...
				if ((base.GetComponent<Animation>().IsPlaying(standAnimation) || !grounded) && SettingsManager.InputSettings.Human.Reload.GetKeyDown() && (!useGun || SettingsManager.LegacyGameSettings.AHSSAirReload.Value || grounded))
				{
					changeBlade();
					return;
				}
			...
			else if (state == HERO_STATE.ChangeBlade)
			{
				if (useGun)
				{
					if (baseAnimation[reloadAnimation].normalizedTime > 0.22f)
					{
						if (!leftGunHasBullet && setup.part_blade_l.activeSelf)
						{
							...
							gameObject4.GetComponent<Renderer>().material = CharacterMaterials.materials[setup.myCostume._3dmg_texture];
							Vector3 force = -baseTransform.forward * 10f + baseTransform.up * 5f - baseTransform.right;
							gameObject4.GetComponent<Rigidbody>().AddForce(force, ForceMode.Impulse);
							Vector3 torque = new Vector3(UnityEngine.Random.Range(-100, 100), UnityEngine.Random.Range(-100, 100), UnityEngine.Random.Range(-100, 100));
							gameObject4.GetComponent<Rigidbody>().AddTorque(torque, ForceMode.Acceleration);
						}
						if (!rightGunHasBullet && setup.part_blade_r.activeSelf)
						{
							...
							gameObject5.GetComponent<Rigidbody>().AddForce(force2, ForceMode.Impulse);
							...
							gameObject5.GetComponent<Rigidbody>().AddTorque(torque2, ForceMode.Acceleration);
						}
					}
				...
				else
				{
					if (!grounded)
					{
						if (base.GetComponent<Animation>()[reloadAnimation].normalizedTime >= 0.2f && !throwedBlades)
						{
							throwedBlades = true;
						...
						if (base.GetComponent<Animation>()[reloadAnimation].normalizedTime >= 0.56f && currentBladeNum > 0)
						{
							setup.part_blade_l.SetActive(true);
							setup.part_blade_r.SetActive(true);
	...
	private void LaunchThunderSpear()
	{
		...
		if (y < 0f && state != HERO_STATE.Land)
		{
			...
			ThunderSpearL.GetComponent<AudioSource>().Play();
		...
		else
		{
			...
			ThunderSpearR.GetComponent<AudioSource>().Play();
		...
		gameObject.GetComponent<Rigidbody>().velocity = vector3 * bombSpeed;
