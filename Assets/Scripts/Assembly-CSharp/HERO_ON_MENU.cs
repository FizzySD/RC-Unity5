...
public class HERO_ON_MENU : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if (component.myCostume.sex == SEX.FEMALE)
		{
			...
			base.GetComponent<Animation>()["stand"].normalizedTime = Random.Range(0f, 1f);
		}
		else
		{
			base.GetComponent<Animation>().Play("stand_levi");
			base.GetComponent<Animation>()["stand_levi"].normalizedTime = Random.Range(0f, 1f);
		}
		float speed = 0.5f;
		base.GetComponent<Animation>()["stand"].speed = speed;
		base.GetComponent<Animation>()["stand_levi"].speed = speed;
	}
}
