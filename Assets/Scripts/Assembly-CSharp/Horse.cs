...
public class Horse : Photon.MonoBehaviour
{
	...
	private void LateUpdate()
	{
		...
		if (State == "mounted")
		{
			...
			myHero.GetComponent<Rigidbody>().velocity = base.GetComponent<Rigidbody>().velocity;
			if (controller.targetDirection != -874f)
			{
				base.gameObject.transform.rotation = Quaternion.Lerp(base.gameObject.transform.rotation, Quaternion.Euler(0f, controller.targetDirection, 0f), 100f * Time.deltaTime / (base.GetComponent<Rigidbody>().velocity.magnitude + 20f));
				if (controller.isWALKDown)
				{
					base.GetComponent<Rigidbody>().AddForce(base.transform.forward * speed * 0.6f, ForceMode.Acceleration);
					if (base.GetComponent<Rigidbody>().velocity.magnitude >= speed * 0.6f)
					{
						base.GetComponent<Rigidbody>().AddForce((0f - speed) * 0.6f * base.GetComponent<Rigidbody>().velocity.normalized, ForceMode.Acceleration);
					}
				}
				else
				{
					base.GetComponent<Rigidbody>().AddForce(base.transform.forward * speed, ForceMode.Acceleration);
					if (base.GetComponent<Rigidbody>().velocity.magnitude >= speed)
					{
						base.GetComponent<Rigidbody>().AddForce((0f - speed) * base.GetComponent<Rigidbody>().velocity.normalized, ForceMode.Acceleration);
					}
				}
				if (base.GetComponent<Rigidbody>().velocity.magnitude > 8f)
				{
					...
					if (!myHero.GetComponent<Animation>().IsPlaying("horse_run"))
					{
				...
				else
				{
					...
					if (!myHero.GetComponent<Animation>().IsPlaying("horse_idle"))
					{
			...
			else
			{
				...
				else if (!myHero.GetComponent<Animation>().IsPlaying("horse_idle"))
				{
					myHero.GetComponent<HERO>().crossFade("horse_idle", 0.1f);
				}
			}
			...
			{
				base.GetComponent<Rigidbody>().AddForce(Vector3.up * 25f, ForceMode.VelocityChange);
			}
		}
		else if (State == "follow")
		{
			...
			{
				base.GetComponent<Rigidbody>().AddForce(base.transform.forward * speed * 0.7f, ForceMode.Acceleration);
				if (base.GetComponent<Rigidbody>().velocity.magnitude >= speed)
				{
					base.GetComponent<Rigidbody>().AddForce((0f - speed) * 0.7f * base.GetComponent<Rigidbody>().velocity.normalized, ForceMode.Acceleration);
				}
			}
			else
			{
