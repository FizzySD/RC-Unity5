...
internal class IN_GAME_MAIN_CAMERA : MonoBehaviour
{
	...
	private void Start()
	{
		...
		base.GetComponent<Camera>().farClipPlane = SettingsManager.GraphicsSettings.RenderDistance.Value;
		createSnapShotRT2();
	}
	...
	public void update2()
	{
		...
		if (triggerAutoLock && lockTarget != null)
		{
			...
			Vector2 vector2 = base.GetComponent<Camera>().WorldToScreenPoint(transform.position - transform.forward * lockTarget.transform.localScale.x);
