	...
	private void OnTriggerStay(Collider other)
	{
		...
		if (type == BottomType.Die)
		{
			...
			if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER)
			{
				...
				{
					other.gameObject.GetComponent<HERO>().netDieLocal(base.GetComponent<Rigidbody>().velocity * 50f, false, -1, string.Empty);
				}
			}
			else
			{
				other.gameObject.GetComponent<HERO>().die(other.gameObject.GetComponent<Rigidbody>().velocity * 50f, false);
			}
		}
