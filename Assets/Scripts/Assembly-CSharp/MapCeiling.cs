...
public class MapCeiling : MonoBehaviour
{
	...
	{
		GameObject gameObject = (GameObject)UnityEngine.Object.Instantiate(FengGameManagerMKII.RCassets.LoadAsset(asset), Vector3.zero, Quaternion.identity);
	...
	{
		if (Camera.main != null && _barrierRef != null && _barrierRef.GetComponent<Renderer>() != null)
		{
			float num = _maxAlpha;
			try
			{
			...
			_color.a = num;
			_barrierRef.GetComponent<Renderer>().material.color = _color;
		}
	}
