...
internal class TITAN : Photon.MonoBehaviour
{
	...
	{
		if (!base.GetComponent<Animation>().IsPlaying(aniName) || IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE || PhotonNetwork.offlineMode)
		{
	...
	public void dieBlowFunc(Vector3 attacker, float hitPauseTime)
	{
		...
		base.GetComponent<Animation>()[hitAnimation].time = 0f;
		base.GetComponent<Animation>()[hitAnimation].speed = 0f;
		needFreshCorePosition = true;
		GameObject.Find("MultiplayerManager").GetComponent<FengGameManagerMKII>().oneTitanDown(string.Empty, false);
	...
	public void lateUpdate()
	{
		...
		{
			if (base.GetComponent<Animation>()["run_walk"].normalizedTime % 1f > 0.1f && base.GetComponent<Animation>()["run_walk"].normalizedTime % 1f < 0.6f && stepSoundPhase == 2)
			{
				stepSoundPhase = 1;
			...
			if (base.GetComponent<Animation>()["run_walk"].normalizedTime % 1f > 0.6f && stepSoundPhase == 1)
			{
				stepSoundPhase = 2;
		...
		{
			if (base.GetComponent<Animation>()["crawler_run"].normalizedTime % 1f > 0.1f && base.GetComponent<Animation>()["crawler_run"].normalizedTime % 1f < 0.56f && stepSoundPhase == 2)
			{
				stepSoundPhase = 1;
			...
			if (base.GetComponent<Animation>()["crawler_run"].normalizedTime % 1f > 0.56f && stepSoundPhase == 1)
			{
				stepSoundPhase = 2;
		...
		{
			if (base.GetComponent<Animation>()["run_abnormal"].normalizedTime % 1f > 0.47f && base.GetComponent<Animation>()["run_abnormal"].normalizedTime % 1f < 0.95f && stepSoundPhase == 2)
			{
				stepSoundPhase = 1;
			...
			if ((base.GetComponent<Animation>()["run_abnormal"].normalizedTime % 1f > 0.95f || base.GetComponent<Animation>()["run_abnormal"].normalizedTime % 1f < 0.47f) && stepSoundPhase == 1)
			{
				stepSoundPhase = 2;
	...
	{
		if (abnormalType == AbnormalType.TYPE_PUNK && myHero != null && myHero.GetComponent<Rigidbody>() != null)
		{
			Vector3 vector = myHero.GetComponent<Rigidbody>().velocity * Time.deltaTime * 30f;
	...
	private void setLevel(float level, int AI, int skinColor)
	{
		...
		base.GetComponent<Rigidbody>().mass *= myLevel;
		base.GetComponent<Rigidbody>().rotation = Quaternion.Euler(0f, UnityEngine.Random.Range(0, 360), 0f);
		if (myLevel > 1f)
		{
			speed *= Mathf.Sqrt(myLevel);
		}
		...
		else if (state == TitanState.attack)
		{
			...
			if (attackAnimation == "throw")
			{
				...
				if (throwRock != null && baseAnimation["attack_" + attackAnimation].normalizedTime >= 0.62f)
				{
					...
					if (myHero != null)
					{
						vector2 = (myHero.transform.position - throwRock.transform.position) / num + myHero.GetComponent<Rigidbody>().velocity;
			...
			if (attackAnimation == "jumper_0" || attackAnimation == "crawler_jump_0")
			{
				if (!attacked)
				{
					if (baseAnimation["attack_" + attackAnimation].normalizedTime >= 0.68f)
					{
						...
						{
							float y2 = myHero.GetComponent<Rigidbody>().velocity.y;
							float num6 = -20f;
							float num7 = gravity;
							...
							Vector3 vector3 = myHero.transform.position + myHero.GetComponent<Rigidbody>().velocity * num11 + Vector3.up * 0.5f * num6 * num11 * num11;
							float y4 = vector3.y;
							float num12;
