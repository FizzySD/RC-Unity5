...
public class TITAN_EREN : Photon.MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		else
		{
			...
			{
				base.GetComponent<Rigidbody>().velocity = Vector3.zero + Vector3.up * base.GetComponent<Rigidbody>().velocity.y;
			...
			else
			{
				...
				{
					currentCamera.GetComponent<Camera>().fieldOfView = Mathf.Lerp(currentCamera.GetComponent<Camera>().fieldOfView, Mathf.Min(100f, base.GetComponent<Rigidbody>().velocity.magnitude), 0.1f);
				}
				else
				{
				...
				if (isAttack || isHit)
				{
					...
					oldCorePosition = base.transform.position - base.transform.Find("Amarture/Core").position;
					base.GetComponent<Rigidbody>().velocity = vector / Time.deltaTime + Vector3.up * base.GetComponent<Rigidbody>().velocity.y;
					base.GetComponent<Rigidbody>().rotation = Quaternion.Lerp(base.gameObject.transform.rotation, Quaternion.Euler(0f, facingDirection, 0f), Time.deltaTime * 10f);
					if (justGrounded)
					{
						justGrounded = false;
					}
				...
				else if (grounded)
				{
					...
					{
						justGrounded = false;
						vector2 = base.GetComponent<Rigidbody>().velocity;
					...
					if (!base.GetComponent<Animation>().IsPlaying("jump_land") && !isAttack && !isHit && !base.GetComponent<Animation>().IsPlaying("born"))
					{
						...
						if (num != 0f || num2 != 0f)
						{
							if (!base.GetComponent<Animation>().IsPlaying("run") && !base.GetComponent<Animation>().IsPlaying("jump_start") && !base.GetComponent<Animation>().IsPlaying("jump_air"))
							{
								crossFade("run", 0.1f);
							}
						}
				...
				else
				{
					...
					{
						playAnimation("jump_air");
						base.GetComponent<Rigidbody>().AddForce(Vector3.up * 240f, ForceMode.VelocityChange);
					}
					if (!base.GetComponent<Animation>().IsPlaying("jump") && !isHit)
					{
						...
						if (num != 0f || num2 != 0f)
						{
							base.GetComponent<Rigidbody>().AddForce(force2, ForceMode.Impulse);
						}
						else
						{
							num8 = -874f;
						...
						{
							base.GetComponent<Rigidbody>().rotation = Quaternion.Lerp(base.gameObject.transform.rotation, Quaternion.Euler(0f, facingDirection, 0f), Time.deltaTime * 6f);
						}
					}
				}
				base.GetComponent<Rigidbody>().AddForce(new Vector3(0f, (0f - gravity) * base.GetComponent<Rigidbody>().mass, 0f));
	...
	{
		if (!isHit && !hasDied && !base.GetComponent<Animation>().IsPlaying("born"))
		{
			if (rockLift)
			{
				crossFade("die", 0.1f);
	...
	private void RockUpdate()
	{
		...
		else if (rockPhase == 4)
		{
			...
			if (waitCounter > 4.2f)
			{
				...
				rock.GetComponent<Animation>()["move"].normalizedTime = base.GetComponent<Animation>()["rock_walk"].normalizedTime;
				waitCounter = 0f;
		...
		else if (rockPhase == 5)
		{
			...
			force2.y = 0f;
			base.GetComponent<Rigidbody>().AddForce(force2, ForceMode.VelocityChange);
			base.GetComponent<Rigidbody>().AddForce(new Vector3(0f, (0f - gravity) * base.GetComponent<Rigidbody>().mass, 0f));
		...
		else
		{
			...
			if (base.GetComponent<Animation>()["rock_fix_hole"].normalizedTime >= 0.62f && !rockHitGround)
			{
				rockHitGround = true;
	...
	public void update()
	{
		...
		{
			if (base.GetComponent<Animation>()["run"].normalizedTime % 1f > 0.3f && base.GetComponent<Animation>()["run"].normalizedTime % 1f < 0.75f && stepSoundPhase == 2)
			{
				stepSoundPhase = 1;
			...
			if (base.GetComponent<Animation>()["run"].normalizedTime % 1f > 0.75f && stepSoundPhase == 1)
			{
				stepSoundPhase = 2;
		...
		if (hasDied)
		{
			if (!(base.GetComponent<Animation>()["die"].normalizedTime >= 1f) && !(hitAnimation == "hit_annie_3"))
			{
				return;
			}
			if (realBody != null)
		...
		else
		{
			...
			if (isHit)
			{
				if (base.GetComponent<Animation>()[hitAnimation].normalizedTime >= 1f)
				{
					isHit = false;
					falseAttack();
					playAnimation("idle");
			...
			if (grounded && !isAttack && !base.GetComponent<Animation>().IsPlaying("jump_land") && !isAttack && !base.GetComponent<Animation>().IsPlaying("born"))
			{
				if (SettingsManager.InputSettings.Shifter.AttackDefault.GetKeyDown() || SettingsManager.InputSettings.Shifter.AttackSpecial.GetKeyDown())
				{
					...
					if (!flag)
					{
						...
						base.GetComponent<Animation>()[attackAnimation].time = 0f;
						isAttack = true;
						needFreshCorePosition = true;
			...
			if (!isAttack)
			{
				if ((grounded || base.GetComponent<Animation>().IsPlaying("idle")) && !base.GetComponent<Animation>().IsPlaying("jump_start") && !base.GetComponent<Animation>().IsPlaying("jump_air") && !base.GetComponent<Animation>().IsPlaying("jump_land") && SettingsManager.InputSettings.Shifter.Jump.GetKey())
				{
			...
			{
				if (base.GetComponent<Animation>()[attackAnimation].time >= 0.1f && SettingsManager.InputSettings.Shifter.AttackDefault.GetKeyDown())
				{
					isNextAttack = true;
				}
				float num = 0f;
				...
				if (hitPause > 0f)
				{
					...
					if (hitPause <= 0f)
					{
						base.GetComponent<Animation>()[attackAnimation].speed = 1f;
						hitPause = 0f;
					}
				}
				if (num3 > 0f && isNextAttack && base.GetComponent<Animation>()[attackAnimation].normalizedTime >= num3)
				{
				...
				if ((base.GetComponent<Animation>()[attackAnimation].normalizedTime >= num && base.GetComponent<Animation>()[attackAnimation].normalizedTime <= num2) || (!attackChkOnce && base.GetComponent<Animation>()[attackAnimation].normalizedTime >= num))
				{
					if (!attackChkOnce)
					{
						if (attackAnimation == "attack_combo_002")
					...
					for (int i = 0; i < array.Length; i++)
					{
						...
						base.GetComponent<Animation>()[attackAnimation].speed = 0f;
						if (attackAnimation == "attack_combo_002")
						{
							hitPause = 0.05f;
				...
				if (base.GetComponent<Animation>()[attackAnimation].normalizedTime >= 1f)
				{
					falseAttack();
					playAnimation("idle");
				}
			...
			{
				if (base.GetComponent<Animation>()["born"].normalizedTime >= 0.28f && !isPlayRoar)
				{
					isPlayRoar = true;
					playSound("snd_eren_roar");
				}
				if (base.GetComponent<Animation>()["born"].normalizedTime >= 0.5f && base.GetComponent<Animation>()["born"].normalizedTime <= 0.7f)
				{
					currentCamera.GetComponent<IN_GAME_MAIN_CAMERA>().startShake(0.5f, 1f);
				}
				if (base.GetComponent<Animation>()["born"].normalizedTime >= 1f)
				{
					crossFade("idle", 0.1f);
					if (IN_GAME_MAIN_CAMERA.gametype != 0)
					{
