...
public class TITAN_SETUP : Photon.MonoBehaviour
{
	...
	public IEnumerator loadskinE(int hair, int eye, string hairlink)
	{
		...
		if (this.hair.hair != string.Empty)
		{
			...
			gameObject.GetComponent<Renderer>().material = CharacterMaterials.materials[this.hair.texture];
	...
	public void setHair2()
	{
		...
		else
		{
			...
			part_hair.GetComponent<Renderer>().material = CharacterMaterials.materials[hair.texture];
			part_hair.GetComponent<Renderer>().material.color = HeroCostume.costume[Random.Range(0, HeroCostume.costume.Length - 5)].hair_color;
			...
			if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER && base.photonView.isMine)
			{
				object[] parameters = new object[5]
				{
					hairType,
					num4,
					part_hair.GetComponent<Renderer>().material.color.r,
					part_hair.GetComponent<Renderer>().material.color.g,
					part_hair.GetComponent<Renderer>().material.color.b
				};
	...
	private void setHairPRC(int type, int eye_type, float c1, float c2, float c3)
	{
		...
		if (hair.hair != string.Empty)
		{
			...
			gameObject.transform.rotation = hair_go_ref.transform.rotation;
			gameObject.transform.localScale = hair_go_ref.transform.localScale;
			gameObject.GetComponent<Renderer>().material = CharacterMaterials.materials[hair.texture];
			gameObject.GetComponent<Renderer>().material.color = new Color(c1, c2, c3);
			part_hair = gameObject;
		}
	...
	public void setPunkHair()
	{
		...
		{
		case 1:
			gameObject.GetComponent<Renderer>().material.color = FengColor.hairPunk1;
			break;
		case 2:
			gameObject.GetComponent<Renderer>().material.color = FengColor.hairPunk2;
			break;
		case 3:
			gameObject.GetComponent<Renderer>().material.color = FengColor.hairPunk3;
			break;
		}
		part_hair = gameObject;
