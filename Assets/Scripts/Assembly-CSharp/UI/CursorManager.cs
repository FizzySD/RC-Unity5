...
namespace UI
{
	internal class CursorManager : MonoBehaviour
	{
		...
		{
			_cursorPointer = (Texture2D)AssetBundleManager.MainAssetBundle.LoadAsset("CursorPointer");
			foreach (CrosshairStyle value2 in Enum.GetValues(typeof(CrosshairStyle)))
			{
				Texture2D value = (Texture2D)AssetBundleManager.MainAssetBundle.LoadAsset("Cursor" + value2);
